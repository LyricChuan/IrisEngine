...
float3 GetPointSphericalCoordinates(float3 InPoint)
{
    ...
    float Phi = atan2(InPoint.z, InPoint.x) * AnglePre;
...
int GetSampleCubeMapIndexR(float3 InPointPosition)
{
    ...
    CubeMapAxialRangeRight.PositiveX = float4(45.f, 135.f, 45.f, -45.f);//Phi 属于 0-45  0--45
    CubeMapAxialRangeRight.NegativeX = float4(45.f, 135.f, 135.f, -135.f);//Phi 属于 135-180 -135--180 //确保它已经转为CubeMapViewport下的坐标
    ...
    else if (IsPointInCubeMapVieport(PointTheta, PointPhi, CubeMapAxialRangeRight.PositiveY, 3,0.f,false))
    {
        return 2;
    }
    else if (IsPointInCubeMapVieport(PointTheta, PointPhi, CubeMapAxialRangeRight.NegativeY, 3, 0.f, false))
    {
        return 3;
    }
    else if (IsPointInCubeMapVieport(PointTheta, PointPhi, CubeMapAxialRangeRight.PositiveZ, 1, 0.f, false))
    {
        return 4;
    }
    else if (IsPointInCubeMapVieport(PointTheta, PointPhi, CubeMapAxialRangeRight.NegativeZ, 2, 0.f, false))
    {
        return 5;
    }
