		...
		for (;!BuffStream.eof();)//eof : 判断是否结束
		{
			...
			if (strlen(TmpLine) > 0)
			{
				...
				else if(TmpLine[0] == 'f')
				{
					...
					char TmpBuff[256] = { 0 };
					for (size_t i = 0; i < 3; i++)//每行有三个顶点索引、三个纹理索引、三个法线索引
					{
						...
						//找到索引的位置
						int StringPosA = find_string(SaveLineString, "/", 0);//第一个"/"位置
						...
						//纹理Index
						int StringPosB = find_string(SaveLineString, "/", StringPosA + 1);//第二个"/"位置
						memset(TmpBuff, 0, 256);
