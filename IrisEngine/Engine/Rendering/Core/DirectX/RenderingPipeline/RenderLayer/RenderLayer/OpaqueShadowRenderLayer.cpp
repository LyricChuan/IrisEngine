...
void FOpaqueShadowRenderLayer::BuildShader()
{
	...
	VertexShader.BuildShaders(L"../IrisEngine/Shader/Shadow.hlsl", "VertexShaderMain", "vs_5_1", D3DShaderMacro.data());
	PixelShader.BuildShaders(L"../IrisEngine/Shader/Shadow.hlsl", "PixelShaderMain", "ps_5_1", D3DShaderMacro.data());
...
void FOpaqueShadowRenderLayer::BuildVientianeShadowShader()
{
	...
	VertexShader.BuildShaders(L"../IrisEngine/Shader/VientianeShadow.hlsl", "VertexShaderMain", "vs_5_1", D3DShaderMacro.data());
	PixelShader.BuildShaders(L"../IrisEngine/Shader/VientianeShadow.hlsl", "PixelShaderMain", "ps_5_1", D3DShaderMacro.data());
	DirectXPipelineState->BindShader(VertexShader, PixelShader);
...
void FOpaqueShadowRenderLayer::BuildPSO()
{
	...
	//构建固体
	//给正交Shadow
	DirectXPipelineState->Build(OrthogonalShadow);//平行光阴影用

	//给透视Shadow
	GPSDesc.RasterizerState.DepthBias = 1000;//固定偏移量
	DirectXPipelineState->Build(PerspectiveShadow);//聚光阴影用

	//万向阴影
	DirectXPipelineState->BuildParam();
	BuildVientianeShadowShader();
	DirectXPipelineState->Build(VientianeShadow);//点光阴影用
}
