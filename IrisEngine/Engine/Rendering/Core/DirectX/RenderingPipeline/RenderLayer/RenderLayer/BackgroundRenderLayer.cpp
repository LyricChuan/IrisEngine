...
void FBackgroundRenderLayer::BuildShader()
{
	...
	VertexShader.BuildShaders(L"../IrisEngine/Shader/Sky.hlsl", "VertexShaderMain", "vs_5_1",  D3DShaderMacro.data());
	PixelShader.BuildShaders(L"../IrisEngine/Shader/Sky.hlsl", "PixelShaderMain", "ps_5_1", D3DShaderMacro.data());
...
void FBackgroundRenderLayer::BuildPSO()
{
	...
	RasterizerDesc.FillMode = D3D12_FILL_MODE_SOLID;//以固体方式显示
	RasterizerDesc.CullMode = D3D12_CULL_MODE_NONE;//设置为不进行裁剪

	DirectXPipelineState->SetRasterizerState(RasterizerDesc);

	CD3DX12_DEPTH_STENCIL_DESC DepthStencilDesc = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT);
	DepthStencilDesc.DepthFunc = D3D12_COMPARISON_FUNC_LESS_EQUAL;//设置小于等于，防止深度模板测试失败
